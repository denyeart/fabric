...
- **Initialization:** If you use the low level APIs provided by the Fabric Chaincode
  ...
  Note that many users choose not to utilize the chaincode lifecycle initialization mechanism.
  Rather, chaincode initialization can be handled like any other application state.
  Chaincode functions often perform checks against existing state,
  and initialization state can be implemented like any other chaincode state.
